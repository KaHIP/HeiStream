/****************************************************************************
 * edge_partitioning_algorithm.cpp
 *****************************************************************************/
...
EdgePartitioningAlgorithm::EdgePartitioningAlgorithm(
...
void EdgePartitioningAlgorithm::prepare_run(Config& config, const std::string& graph_filename,
                                            StreamContext& ctx) {
    (void)ctx;
...
void EdgePartitioningAlgorithm::run(Config& config, const std::string& graph_filename,
                                    StreamContext& ctx) {
    core::pipeline::execute_algorithm_pipeline(config, graph_filename, ctx, *scheduler_, *evaluator_,
                                               *reporter_);
}

void EdgePartitioningAlgorithm::finalize_run(Config& config, const std::string& graph_filename,
                                             StreamContext& ctx) {
    (void)config;
    (void)graph_filename;
    ctx.edge_pass_state.clear_runtime();
}
