...
class configuration {
   public:
    ...
    void strong_separator(Config& config);
    void eco_separator(Config& config);
    void fast_separator(Config& config);

    void standard(Config& config);
    void standardsnw(Config& config);

    void fastsocial(Config& config);
    void ecosocial(Config& config);
    void strongsocial(Config& config);

    void integrated_mapping(Config& partition_config);
    void stream_partition(Config& partition_config);

    // void fastsocial_separator( Config & config );
    // void ecosocial_separator( Config & config );
    // void strongsocial_separator( Config & config );
};

inline void configuration::strong(Config& partition_config) {
...
inline void configuration::eco(Config& partition_config) {
    standard(partition_config);
...
inline void configuration::fast(Config& partition_config) {
    standard(partition_config);
...
inline void configuration::strong_separator(Config& partition_config) {
    standard(partition_config);
    partition_config.matching_type = MATCHING_GPA;
    partition_config.permutation_quality = PERMUTATION_QUALITY_GOOD;
    partition_config.permutation_during_refinement = PERMUTATION_QUALITY_GOOD;
...
inline void configuration::eco_separator(Config& partition_config) {
    standard(partition_config);
    partition_config.eco = true;
    partition_config.aggressive_random_levels = std::max(2, (int)(7 - log2(partition_config.k)));
...
inline void configuration::fast_separator(Config& partition_config) {
    standard(partition_config);
...
inline void configuration::standard(Config& partition_config) {
    ...
    // Stream Partition
    partition_config.run_parallel = false;
    partition_config.param_dbl1 = 0;
    partition_config.param_enbld1 = false;
    ...
    partition_config.inv_ghost_weight = 0.0f;
    ...
    partition_config.batch_manager = nullptr;
    ...
    partition_config.num_split_edges = std::numeric_limits<NodeID>::max();
...
inline void configuration::standardsnw(Config& partition_config) {
...
// inline void configuration::fastsocial_separator( Config & partition_config ) {
...
// inline void configuration::ecosocial_separator( Config & partition_config ) {
// eco(partition_config);
// standardsnw(partition_config);
// partition_config.label_propagation_refinement = false;
// partition_config.global_cycle_iterations      = 3;
...
// inline void configuration::strongsocial_separator( Config & partition_config ) {
...
inline void configuration::fastsocial(Config& partition_config) {
    eco(partition_config);
    standardsnw(partition_config);
    partition_config.label_propagation_refinement = true;
    partition_config.cluster_coarsening_during_ip = true;
...
inline void configuration::ecosocial(Config& partition_config) {
    eco(partition_config);
    standardsnw(partition_config);
    partition_config.label_propagation_refinement = false;
    partition_config.global_cycle_iterations = 3;
...
inline void configuration::strongsocial(Config& partition_config) {
    strong(partition_config);
    standardsnw(partition_config);
...
inline void configuration::integrated_mapping(Config& partition_config) {
    partition_config.fm_search_limit = 0;
    // partition_config.kway_fm_search_limit                   = 0;
    partition_config.corner_refinement_enabled = false;
...
inline void configuration::stream_partition(Config& partition_config) {
