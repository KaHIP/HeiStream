...
class balance_configuration {
   public:
    ...
    void configurate_balance(Config& partition_config, graph_access& G,
        ...
        if (partition_config.balance_edges && partition_config.imbalance != 0) {
            // balancing edges is disabled for the perfectly balanced case since this case requires
        ...
        if (partition_config.imbalance == 0 && !partition_config.kaffpaE) {
            partition_config.upper_bound_partition =
                (1 + epsilon + 0.01) * ceil(largest_graph_weight / (double)partition_config.k);
            partition_config.kaffpa_perfectly_balance = true;
        } else {
            NodeWeight load = largest_graph_weight + edge_weights;
            partition_config.upper_bound_partition =
                (1 + epsilon) * ceil(load / (double)partition_config.k);
        }

        if (partition_config.adapt_bal) {
            partition_config.glob_block_upperbound =
                (1 + epsilon) * largest_graph_weight / (double)partition_config.k;
            ...
            if (partition_config.enable_mapping || partition_config.integrated_mapping) {
                ...
                for (unsigned i = 1; i < partition_config.interval_sizes.size(); i++) {
                    partition_config.interval_sizes[i] =
                        partition_config.group_sizes[i] * partition_config.interval_sizes[i - 1];
                }
            }
        }
