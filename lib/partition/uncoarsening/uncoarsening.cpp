...
int uncoarsening::perform_uncoarsening(Config& config, graph_hierarchy& hierarchy) {
    return perform_uncoarsening_cut(config, hierarchy);
}

int uncoarsening::perform_uncoarsening_cut(Config& config, graph_hierarchy& hierarchy) {
    int improvement = 0;

    Config cfg = config;
    refinement* refine = nullptr;

    refine = new mixed_refinement();

    graph_access* coarsest = hierarchy.get_coarsest();
    PRINT(std::cout << "log>" << "unrolling graph with " << coarsest->number_of_nodes() << '\n';)

    complete_boundary* finer_boundary = nullptr;
    complete_boundary* coarser_boundary = nullptr;
    double factor = config.balance_factor;
    cfg.upper_bound_partition =
        ((!hierarchy.isEmpty()) * factor + 1.0) * config.upper_bound_partition;

    improvement += (int)refine->perform_refinement(cfg, *coarsest, *coarser_boundary);

    graph_access* to_delete = nullptr;
    ...
    while (!hierarchy.isEmpty()) {
        ...
        PRINT(std::cout << "log>" << "unrolling graph with " << G->number_of_nodes() << '\n';)

        // call refinement
        double cur_factor = factor / (hierarchy_deepth - hierarchy.size());
        cfg.upper_bound_partition =
            ((!hierarchy.isEmpty()) * cur_factor + 1.0) * config.upper_bound_partition;
        PRINT(std::cout << "cfg upperbound " << cfg.upper_bound_partition << '\n';)
