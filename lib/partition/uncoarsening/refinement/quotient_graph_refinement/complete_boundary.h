...
class complete_boundary {
   public:
    ...
    void build();
    void build_from_coarser(complete_boundary* coarser_boundary, NodeID coarser_no_nodes,
    ...
    void postMovedBoundaryNodeUpdates(NodeID target, boundary_pair* pair, bool update_edge_cuts,
                                      bool update_all_boundaries);
    void balance_singletons(const Config& config, graph_access& G);
    void balance_singletons_het_bal(const Config& config, graph_access& G);
    ...
    inline PartialBoundary& getDirectedBoundary(PartitionID partition, PartitionID lhs,
                                                PartitionID rhs);

    inline void setup_start_nodes(graph_access& G, PartitionID partition, boundary_pair& bp,
    ...
    // Lightweight methods for streaming partitioning
...
inline void complete_boundary::build_from_coarser(complete_boundary* coarser_boundary,
    ...
    forall_nodes(G, n) {
        ...
        if (!coarse_is_border_node[coarse_node])
            continue;
...
inline void complete_boundary::insert(NodeID node, PartitionID insert_node_into,
                                      boundary_pair* pair) {
...
inline PartialBoundary& complete_boundary::getDirectedBoundary(PartitionID partition,
...
void complete_boundary::setup_start_nodes(graph_access& G, PartitionID partition, boundary_pair& bp,
                                          boundary_starting_nodes& start_nodes) {
...
inline void complete_boundary::fastComputeQuotientGraph(graph_access& Q_bar,
                                                        const NodeID& no_of_blocks) {
    basicGraph* graphref = new basicGraph;
    ...
    std::vector<NodeWeight> block_ghost_weight(no_of_blocks, 0);
    std::vector<EdgeWeight> target_edgeweight(no_of_blocks, 0);
    std::vector<PartitionID> list_targets;

    if (m_graph_ref->has_compressed_ghost_nodes()) {
        ...
        forall_nodes((*m_graph_ref), node) {
            ...
            block_ghost_weight[source_partition] += m_graph_ref->getImplicitGhostNodes(node);
        }
        endfor
    } else {
        // Fill out list of nodes per block
    ...
    if (m_graph_ref->has_compressed_ghost_nodes()) {
        Q_bar.resizeImplicitGhostNodesList(no_of_blocks);
        for (unsigned p = 0; p < no_of_blocks; p++) {
            Q_bar.setImplicitGhostNodes(p, block_ghost_weight[p]);
        }
    }
...
inline void complete_boundary::getNeighbors(PartitionID& block,
                                            std::vector<PartitionID>& neighbors) {
    // lazy
    if (Q.graphref == nullptr) {
...
void complete_boundary::setup_start_nodes_around_blocks(graph_access& G, PartitionID& lhs,
                                                        PartitionID& rhs,
                                                        boundary_starting_nodes& start_nodes) {
    ...
    for (unsigned i = 0; i < lhs_neighbors.size(); i++) {
        ...
        forall_boundary_nodes(partial_boundary_lhs, cur_bnd_node) {
            ...
            PartialBoundary& partial_boundary_neighbor =
                getDirectedBoundary(neighbor, lhs, neighbor);
    ...
    for (unsigned i = 0; i < rhs_neighbors.size(); i++) {
        ...
        forall_boundary_nodes(partial_boundary_rhs, cur_bnd_node) {
            ...
            PartialBoundary& partial_boundary_neighbor =
                getDirectedBoundary(neighbor, rhs, neighbor);
        forall_boundary_nodes(partial_boundary_neighbor, cur_bnd_node) {
            ASSERT_EQ(G.getPartitionIndex(cur_bnd_node), neighbor);
            if (allready_contained.find(cur_bnd_node) == allready_contained.end()) {
                start_nodes.push_back(cur_bnd_node);
...
inline bool complete_boundary::assert_boundaries_are_bnodes() {
    ...
    endfor QuotientGraphEdges qgraph_edges;
    getQuotientGraphEdges(qgraph_edges);
    for (unsigned i = 0; i < qgraph_edges.size(); i++) {
        boundary_pair& pair = qgraph_edges[i];
        ASSERT_NEQ(pair.lhs, pair.rhs);
