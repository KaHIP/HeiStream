...
void complete_boundary::postMovedBoundaryNodeUpdates(NodeID node, boundary_pair* pair,
    ...
    // First delete this node from all incidient partition boudnary and decreas the edgecut (from,
    // target_partition != to) then insert it in the right target
        ...
        if (update_all_boundaries || targetPartition != to) {
            ...
            if (targetPartition != from) {
                ...
                forall_out_edges(G, t_e, target) {
                    ...
                    if (!target_is_still_incident) deleteNode(target, targetPartition, &delete_bp);
...
void complete_boundary::balance_singletons(const Config& config, graph_access& G) {
    for (unsigned i = 0; i < m_singletons.size(); i++) {
        ...
        if (config.stream_input && node >= G.number_of_nodes() - config.quotient_nodes) {
            continue;
        }
...
void complete_boundary::balance_singletons_het_bal(const Config& config, graph_access& G) {
    for (unsigned i = 0; i < m_singletons.size(); i++) {
