...
void contraction::contract(const Config& partition_config, graph_access& G, graph_access& coarser,
    ...
    forall_edges(G, e) {
        ...
        std::vector<EdgeID>
            edge_positions(no_of_coarse_vertices, UNDEFINED_EDGE);
...
void contraction::contract_clustering(const Config& partition_config, graph_access& G,
...
// for documentation see technical reports of christian schulz
void contraction::contract_partitioned(const Config& partition_config, graph_access& G,
                                       graph_access& coarser, const Matching& edge_matching,
                                       const CoarseMapping& coarse_mapping,
                                       const NodeID& no_of_coarse_vertices,
                                       const NodePermutationMap& permutation) const {
    ...
    PRINT(std::cout << "contracting a partitioned graph" << '\n';)
    forall_nodes(G, n) {
        NodeID node = permutation[n];
        // we look only at the coarser nodes
        if (coarse_mapping[node] != cur_no_vertices)
