...
class coarsening_configurator {
   public:
    ...
    void configure_coarsening(const Config& partition_config, matching** edge_matcher,
                              unsigned level);
};

inline void coarsening_configurator::configure_coarsening(const Config& partition_config,
                                                          matching** edge_matcher, unsigned level) {
    switch (partition_config.matching_type) {
        ...
        case MATCHING_GPA:
            *edge_matcher = new gpa_matching();
            PRINT(std::cout << "gpa matching" << '\n';)
            break;
        case MATCHING_RANDOM_GPA:
            PRINT(std::cout << "random gpa matching" << '\n';)
            *edge_matcher = new gpa_matching();
            break;
        case CLUSTER_COARSENING:
            PRINT(std::cout << "cluster_coarsening" << '\n';)
    ...
    if (partition_config.matching_type == MATCHING_RANDOM_GPA &&
        ...
        delete *edge_matcher;
        PRINT(std::cout << "random matching" << '\n';)
        *edge_matcher = new random_matching();
    }
}
