...
class separator_simple_stop_rule : public stop_rule {
   public:
    separator_simple_stop_rule(Config& config, NodeID number_of_nodes) {
...
inline bool separator_simple_stop_rule::stop(NodeID no_of_finer_vertices,
...
class simple_stop_rule : public stop_rule {
   public:
    simple_stop_rule(Config& config, NodeID number_of_nodes) {
        double x = 60;
...
class multiple_k_stop_rule : public stop_rule {
   public:
    multiple_k_stop_rule(Config& config, NodeID number_of_nodes) {
        ...
        if (config.disable_max_vertex_weight_constraint) {
            ...
            if (config.initial_partitioning) {
                ...
                config.max_vertex_weight =
                    1.5 * ((double)config.work_load) / (2 * config.num_vert_stop_factor);
            } else {
                config.max_vertex_weight = (NodeWeight)(1.5 * config.work_load / num_stop);
            }
        }
...
class multibfs_stop_rule : public stop_rule {
   public:
    multibfs_stop_rule(Config& config, NodeID number_of_nodes) {
        num_stop = std::max(number_of_nodes / (2.0 * config.xxx * config.k),
                            (double)config.xxx * config.k);
        if (config.disable_max_vertex_weight_constraint) {
            config.max_vertex_weight = config.upper_bound_partition;
        } else {
            config.max_vertex_weight = (NodeWeight)(1.5 * config.work_load / num_stop);
...
inline bool multibfs_stop_rule::stop(NodeID no_of_finer_vertices, NodeID no_of_coarser_vertices) {
    double contraction_rate = 1.0 * no_of_finer_vertices / (double)no_of_coarser_vertices;
    return contraction_rate >= 1.1 && no_of_coarser_vertices >= num_stop;
}
