...
edge_ratings::edge_ratings(const Config& _partition_config) : partition_config(_partition_config) {}
...
forall_nodes(G, node){forall_out_edges(G, e, node){dist[e] += 0.0001;
}
endfor
}
endfor
...
void edge_ratings::rate_realweight(graph_access& G) {
    forall_nodes(G, node){forall_out_edges(G, e, node){EdgeRatingType rating = G.getEdgeWeight(e);
    G.setEdgeRating(e, rating);
}
endfor
}
