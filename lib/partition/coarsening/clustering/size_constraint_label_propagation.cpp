...
void size_constraint_label_propagation::match(const Config& partition_config, graph_access& G,
                                              Matching& _matching, CoarseMapping& coarse_mapping,
    ...
    if (partition_config.ensemble_clusterings) {
        ensemble_clusterings(partition_config, G, _matching, coarse_mapping, no_of_coarse_vertices,
                             permutation);
    } else {
        match_internal(partition_config, G, _matching, coarse_mapping, no_of_coarse_vertices,
                       permutation);
    }
}

void size_constraint_label_propagation::match_internal(const Config& partition_config,
                                                       graph_access& G, Matching& _matching,
                                                       CoarseMapping& coarse_mapping,
                                                       NodeID& no_of_coarse_vertices,
                                                       NodePermutationMap& permutation) {
    std::vector<NodeID> cluster_id(G.number_of_nodes());
    NodeWeight block_upperbound = ceil(partition_config.upper_bound_partition /
                                       (double)partition_config.cluster_coarsening_factor);
...
void size_constraint_label_propagation::ensemble_clusterings(const Config& partition_config,
                                                             graph_access& G, Matching& _matching,
                                                             CoarseMapping& coarse_mapping,
                                                             NodeID& no_of_coarse_vertices,
                                                             NodePermutationMap& permutation) {
    ...
    for (int i = 0; i < runs; i++) {
        ...
        if (i != 0) {
            ensemble_two_clusterings(G, cur_cluster, ensemble_cluster, ensemble_cluster,
                                     no_of_coarse_vertices);
        } else {
            forall_nodes(G, node) {
...
void size_constraint_label_propagation::label_propagation(const Config& partition_config,
                                                          graph_access& G,
...
void size_constraint_label_propagation::label_propagation(const Config& partition_config,
        ...
        for (NodeID node : permutation) {
            ...
            forall_out_edges(G, e, node) {
                ...
                if (target >= G.number_of_nodes() - partition_config.quotient_nodes ||
                    (partition_config.graph_allready_partitioned &&
    ...
    if (isolated_nodes.size() >= 2) {
        cluster_isolated_nodes(partition_config, G, cluster_id, cluster_sizes, block_upperbound,
                               isolated_nodes);
    }

    // std::cout << "partition_config.remaining_stream_nodes: " <<
    // partition_config.remaining_stream_nodes << "\n";
    remap_cluster_ids(partition_config, G, cluster_id, no_of_blocks);
}

void size_constraint_label_propagation::create_coarsemapping(const Config& partition_config,
                                                             graph_access& G,
                                                             std::vector<NodeID>& cluster_id,
                                                             CoarseMapping& coarse_mapping) {
    forall_nodes(G, node) {
...
void size_constraint_label_propagation::remap_cluster_ids(const Config& partition_config,
                                                          graph_access& G,
                                                          std::vector<NodeID>& cluster_id,
                                                          NodeID& no_of_coarse_vertices,
                                                          bool apply_to_graph) {
    ...
    forall_nodes(G, node) {
        ...
        endfor G.set_partition_count(cur_no_clusters);
    }

    no_of_coarse_vertices = cur_no_clusters;
}

void size_constraint_label_propagation::cluster_isolated_nodes(
    const Config& partition_config, graph_access& G, std::vector<NodeID>& cluster_id,
    std::vector<NodeWeight>& cluster_sizes, const NodeWeight& block_upperbound,
    std::vector<NodeID>& isolated_nodes) {
    ...
    for (NodeID it = 0; it < isolated_nodes.size(); it++) {
        ...
        for (int it2 = it + 1; it2 < isolated_nodes.size(); it2++) {
            ...
            if ((cluster_sizes[cur_block] + G.getNodeWeight(node) <= block_upperbound ||
                 cur_block == cluster_id[node]) &&
                (!partition_config.graph_allready_partitioned ||
                 G.getPartitionIndex(node) == G.getPartitionIndex(target)) &&
                (!partition_config.combine ||
                 G.getSecondPartitionIndex(node) == G.getSecondPartitionIndex(target))) {
