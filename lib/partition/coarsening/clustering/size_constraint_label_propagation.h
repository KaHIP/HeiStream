...
typedef std::unordered_map<const ensemble_pair, data_ensemble_pair, hash_ensemble_pair,
                           compare_ensemble_pair>
    hash_ensemble;
...
class size_constraint_label_propagation : public matching {
   public:
    ...
    void ensemble_clusterings(const Config& config, graph_access& G, Matching& _matching,
                              CoarseMapping& coarse_mapping, NodeID& no_of_coarse_vertices,
                              NodePermutationMap& permutation);

    void ensemble_two_clusterings(graph_access& G, std::vector<NodeID>& lhs,
                                  std::vector<NodeID>& rhs, std::vector<NodeID>& output,
                                  NodeID& no_of_coarse_vertices);

    void match_internal(const Config& config, graph_access& G, Matching& _matching,
                        CoarseMapping& coarse_mapping, NodeID& no_of_coarse_vertices,
                        NodePermutationMap& permutation);

    void remap_cluster_ids(const Config& partition_config, graph_access& G,
                           std::vector<NodeID>& cluster_id, NodeID& no_of_coarse_vertices,
                           bool apply_to_graph = false);

    void cluster_isolated_nodes(const Config& partition_config, graph_access& G,
    ...
    void create_coarsemapping(const Config& partition_config, graph_access& G,
                              std::vector<NodeID>& cluster_id, CoarseMapping& coarse_mapping);

    void label_propagation(const Config& partition_config, graph_access& G,
                           const NodeWeight& block_upperbound,
                           std::vector<NodeID>& cluster_id,  // output paramter
                           NodeID& number_of_blocks);        // output parameter

    void label_propagation(const Config& partition_config, graph_access& G,
                           std::vector<NodeID>& cluster_id, NodeID& number_of_blocks);
};
