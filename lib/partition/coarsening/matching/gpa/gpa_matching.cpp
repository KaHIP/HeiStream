...
void gpa_matching::match(const Config& partition_config, graph_access& G, Matching& edge_matching,
                         CoarseMapping& coarse_mapping, NodeID& no_of_coarse_vertices,
                         NodePermutationMap& permutation) {
    PRINT(std::cout << "matching using gpa" << '\n';)
    ...
    if (partition_config.edge_rating_tiebreaking) {
        Config gpa_perm_config = partition_config;
    ...
    forall_edges(G, e) {
        ...
        if (G.getNodeWeight(source) + G.getNodeWeight(target) >
            partition_config.max_vertex_weight) {
            continue;
        }
...
void gpa_matching::init(graph_access& G, const Config& partition_config,
                        NodePermutationMap& permutation, Matching& edge_matching,
                        std::vector<EdgeID>& edge_permutation, std::vector<NodeID>& sources) {
    forall_nodes(G, n) {
        permutation[n] = n;
        edge_matching[n] = n;
...
void gpa_matching::extract_paths_apply_matching(graph_access& G, std::vector<NodeID>& sources,
                                                Matching& edge_matching, path_set& pathset) {
...
void gpa_matching::apply_matching(graph_access& G, std::vector<EdgeID>& matched_edges,
...
void gpa_matching::unpack_path(const path& p, const path_set& pathset,
...
template <typename VectorOrDeque>
void gpa_matching::dump_unpacked_path(graph_access& G, VectorOrDeque& unpacked_path,
