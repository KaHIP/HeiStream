...
void random_matching::match(const Config& partition_config, graph_access& G,
                            Matching& edge_matching, CoarseMapping& coarse_mapping,
    ...
    forall_nodes(G, n) {
        ...
        if (partition_config.graph_allready_partitioned) {  // in this case edges between partitions
                                                            // arent matched
        forall_nodes(G, n) {
                ...
                forall_out_edges(G, e, curNode) {
                    ...
                    if (edge_matching[target] == target &&
                        ...
                        if (partition_config.combine) {
                            if (G.getSecondPartitionIndex(curNode) !=
    ...
    else {// copy n paste from the first if clause but this time all edges are matchable
          forall_nodes(G, n){NodeID curNode = permutation[n];
    NodeWeight curNodeWeight = G.getNodeWeight(curNode);
...
PRINT(std::cout << "log>" << "no of coarse nodes: " << no_of_coarse_vertices << '\n';)
}
