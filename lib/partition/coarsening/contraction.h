...
class contraction {
   public:
    ...
    void contract(const Config& partition_config, graph_access& finer, graph_access& coarser,
                  const Matching& edge_matching, const CoarseMapping& coarse_mapping,
                  const NodeID& no_of_coarse_vertices, const NodePermutationMap& permutation) const;

    void contract_clustering(const Config& partition_config, graph_access& finer,
                             graph_access& coarser, const Matching& edge_matching,
                             const CoarseMapping& coarse_mapping,
                             const NodeID& no_of_coarse_vertices,
                             const NodePermutationMap& permutation) const;

    void contract_partitioned(const Config& partition_config, graph_access& G,
                              graph_access& coarser, const Matching& edge_matching,
                              const CoarseMapping& coarse_mapping,
                              const NodeID& no_of_coarse_vertices,
                              const NodePermutationMap& permutation) const;
   ...
   private:
    ...
    void visit_edge(graph_access& G, graph_access& coarser, std::vector<EdgeID>& edge_positions,
                    const NodeID coarseNode, const EdgeID e,
                    const std::vector<NodeID>& new_edge_targets) const;
};

inline void contraction::visit_edge(graph_access& G, graph_access& coarser,
                                    std::vector<EdgeID>& edge_positions, const NodeID coarseNode,
                                    const EdgeID e,
                                    const std::vector<NodeID>& new_edge_targets) const {
    EdgeID new_coarse_edge_target = new_edge_targets[e];
    if (new_coarse_edge_target == coarseNode)
        return;  // this is the matched edge ... return
