...
// construction etc. is encapsulated in basicGraph / access to properties etc. is encapsulated in
// graph_access
class basicGraph {
...
// makros - graph access
#define forall_edges(G, e) \
    {                      \
        for (EdgeID e = 0, end = G.number_of_edges(); e < end; ++e) {
#define forall_nodes(G, n) \
    {                      \
...
#define forall_out_edges_starting_at(G, e, n, e_bar) \
    {                                                \
        for (EdgeID e = e_bar, end = G.get_first_invalid_edge(n); e < end; ++e) {
#define forall_blocks(G, p) \
    {                       \
        for (PartitionID p = 0, end = G.get_partition_count(); p < end; p++) {
#define endfor \
    }          \
    }
...
inline EdgeWeight graph_access::getWeightedNodeDegree(NodeID node) {
    EdgeWeight degree = 0;
    for (unsigned e = graphref->m_nodes[node].firstEdge; e < graphref->m_nodes[node + 1].firstEdge;
         ++e) {
...
inline EdgeWeight graph_access::getMaxDegree() {
    if (!m_max_degree_computed) {
        ...
        endfor m_max_degree_computed = true;
    }
...
inline int* graph_access::UNSAFE_metis_style_xadj_array() {
    ...
    endfor xadj[graphref->number_of_nodes()] =
...
inline int graph_access::build_from_metis_weighted(int n, int* xadj, int* adjncy, int* vwgt,
                                                   int* adjwgt) {
    graphref = new basicGraph();
    start_construction(n, xadj[n]);
