...
graph_hierarchy::graph_hierarchy()
    : m_current_coarser_graph(nullptr), m_current_coarse_mapping(nullptr) {}

graph_hierarchy::~graph_hierarchy() {
    for (unsigned i = 0; i < m_to_delete_mappings.size(); i++) {
        if (m_to_delete_mappings[i] != nullptr)
            delete m_to_delete_mappings[i];
    }

    for (unsigned i = 0; i + 1 < m_to_delete_hierachies.size(); i++) {
        if (m_to_delete_hierachies[i] != nullptr)
...
graph_access* graph_hierarchy::pop_finer_and_project(Config& config, bool delta_gains) {
