...
float quality_metrics::getFennelWeight(Config& partition_config) {
    float fennel_weight = 2;
    float fennel_tmp = 0;
    const float remaining_stream_nodes = std::max(
        0.0f,
        static_cast<float>(partition_config.total_nodes - partition_config.stream_assigned_nodes));
    switch (partition_config.fennel_dynamics) {
        ...
        case FENNELADP_LINEAR:
            fennel_weight = 2 * remaining_stream_nodes / (float)partition_config.total_stream_edges;
            break;
        case FENNELADP_MID_LINEAR:
            fennel_tmp = 2 * remaining_stream_nodes / (float)partition_config.total_stream_edges;
            if (fennel_tmp <= 1) {
                fennel_weight = 2 * (fennel_tmp);
            }
            break;
        case FENNELADP_QUADRATIC:
            fennel_tmp = remaining_stream_nodes / (float)partition_config.total_stream_edges;
...
EdgeWeight quality_metrics::fennel_objective(Config& partition_config, graph_access& G,
                                             int* partition_map, double fennel_gamma,
                                             double fennel_alpha) {
    ...
    endfor double alpha_gamma = fennel_gamma * fennel_alpha;
    double objective = 0;
    for (int i = 0; i < k; i++) {
        objective += alpha_gamma * weights[i] *
                     random_functions::approx_sqrt(
                         weights[i]);  // exponent is gamma +1 because each node contributes
    }
...
EdgeWeight quality_metrics::ghost_edge_cut(const Config& config, graph_access& G) {
    (void)config;
    (void)G;
    return 0;
}
...
EdgeWeight quality_metrics::edge_cut(graph_access& G) {
    ...
    endfor return edgeCut / 2;
}

EdgeWeight quality_metrics::edge_cut_full_stream(
    const Config& config, graph_access& G,
...
EdgeWeight quality_metrics::edge_cut_connected(graph_access& G, int* partition_map) {
    ...
    endfor union_find uf(G.number_of_nodes());
    forall_nodes(G, node){forall_out_edges(G, e, node){NodeID target = G.getEdgeTarget(e);
...
forall_nodes(G, node) {
    size_right[uf.Find(node)] = 1;
        ...
        std::cout
    << "number of connected comp " << size_right.size() << '\n';
...
EdgeWeight quality_metrics::max_communication_volume(graph_access& G, int* partition_map) {
    ...
    forall_nodes(G, node) {
        ...
        endfor block_volume[block] += num_incident_blocks;
    }
    endfor
...
int quality_metrics::boundary_nodes(graph_access& G) {
    ...
    endfor return no_of_boundary_nodes;
}
...
double quality_metrics::edge_balance(graph_access& G,
                                     const std::vector<PartitionID>& edge_partition) {
    std::vector<PartitionID> part_weights(G.get_partition_count(), 0);
...
EdgeWeight quality_metrics::objective(const Config& config, graph_access& G, int* partition_map) {
